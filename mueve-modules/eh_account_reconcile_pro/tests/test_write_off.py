...
class TestWriteOff(EhReconcileIntegrationTestCase):
    ...
    def test_write_off_clears_line(self):
        ...
        self.assertNotIn(self.account_expense, line.move_id.line_ids.mapped('account_id'))
    ...
    def test_fx_writeoff_clears_to_exchange_account(self):
        gain = self._ensure_account(self.env, '7800', 'FX Gain', 'income_other')
        loss = self._ensure_account(self.env, '8800', 'FX Loss', 'expense')
        ...
        self.assertFalse(line.move_id.line_ids.filtered(lambda l: l.account_id in (gain | loss)))
