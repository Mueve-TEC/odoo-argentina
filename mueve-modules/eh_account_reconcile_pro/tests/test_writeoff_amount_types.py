...
class TestWriteoffAmountTypes(EhReconcileIntegrationTestCase):
    ...
    def test_residual_returns_full_amount(self):
        ...
        self.assertAlmostEqual(rule.compute_writeoff_amount(line), 100.0, places=2)

    def test_fixed_amount(self):
        rule = self._rule('wo_fixed', 'fixed', 25.0)
        line = self.make_statement_line(100.0)
        self.assertAlmostEqual(rule.compute_writeoff_amount(line), 25.0, places=2)
    ...
    def test_percentage(self):
        ...
        line = self.make_statement_line(100.0)
        self.assertAlmostEqual(rule.compute_writeoff_amount(line), 10.0, places=2)
