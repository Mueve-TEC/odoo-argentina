...
class TestPredictor(EhReconcileIntegrationTestCase):
    def _train(self, payment_ref, account, partner):
        ...
        train_line = self.make_statement_line(100.0, partner=partner, payment_ref=payment_ref)
    ...
    def test_audit_rows_are_append_only(self):
        ...
        aml = self._train('ACME APPEND ONLY', self.account_expense, self.partner_a)
        audit = self.env['eh.reconciliation.audit'].search([('aml_id', '=', aml.id)], limit=1)
        self.assertTrue(audit)
        with self.assertRaises(UserError):
            audit.write({'confidence': 0.0})
        # The old context flag must no longer bypass the guard.
        with self.assertRaises(UserError):
            audit.with_context(eh_internal_audit_write=True).write({'decision': 'skip'})
        with self.assertRaises(UserError):
            audit.unlink()
        with self.assertRaises(UserError):
    ...
    def test_predicts_account_and_partner_from_history(self):
        self._train('ACME RENT MONTHLY FEE', self.account_expense, self.partner_a)
    ...
    def test_no_prediction_without_overlap(self):
        self._train('ACME RENT MONTHLY FEE', self.account_expense, self.partner_a)
        engine = self.env['eh.reconciliation.suggestion.engine']
        new_line = self.make_statement_line(100.0, payment_ref='UNRELATED PAYROLL TRANSFER')
