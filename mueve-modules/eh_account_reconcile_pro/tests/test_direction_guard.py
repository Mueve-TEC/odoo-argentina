...
class TestDirectionGuard(EhReconcileIntegrationTestCase):
    ...
    def setUpClass(cls):
        ...
        if not suspense:
            suspense = cls._ensure_account(cls.env, '1990', 'Bank Suspense', 'asset_current')
    ...
    def make_open_payable_line(cls, partner, amount, date=None, ref=None):
        ...
        move = cls.post_balanced_move(
            [
                ...
                {'account': cls.account_payable, 'credit': amount, 'partner': partner},
            ],
            date=date,
        )
        if ref:
            move.ref = ref
        return move.line_ids.filtered(lambda l: l.account_id == cls.account_payable)

    @classmethod
    def make_credit_note_receivable_line(cls, partner, amount, date=None, ref=None):
        ...
        move = cls.post_balanced_move(
            [
                {'account': cls.account_receivable, 'credit': amount, 'partner': partner},
    ...
    def test_inbound_line_rejects_payable_candidate(self):
        ...
        session = self.env['eh.reconciliation.session'].open_or_create(self.bank_journal.id)
        line = self.make_statement_line(500.0, partner=partner, payment_ref='Customer deposit')
        ...
        self.assertEqual(
            self._posted_move_ids_on(self.account_payable),
            moves_before,
            "A reclassification entry was posted onto the payable account " "for a wrong-side match.",
        )
    ...
    def test_outbound_line_rejects_receivable_candidate(self):
        ...
        session = self.env['eh.reconciliation.session'].open_or_create(self.bank_journal.id)
        line = self.make_statement_line(-400.0, partner=partner, payment_ref='Supplier payment')
    ...
    def test_auto_reconcile_skips_wrong_side_candidate(self):
        ...
        session = self.env['eh.reconciliation.session'].open_or_create(self.bank_journal.id)
        line = self.make_statement_line(750.0, partner=partner, payment_ref='REF750', journal=self.bank_journal)
    ...
    def test_outbound_line_matches_payable_candidate(self):
        ...
        session = self.env['eh.reconciliation.session'].open_or_create(self.bank_journal.id)
        line = self.make_statement_line(-300.0, partner=partner, payment_ref='Bill payment')
    ...
    def test_inbound_line_matches_receivable_candidate(self):
        ...
        session = self.env['eh.reconciliation.session'].open_or_create(self.bank_journal.id)
        line = self.make_statement_line(250.0, partner=partner, payment_ref='Invoice receipt')
    ...
    def test_inbound_line_matches_invoice_plus_credit_note_group(self):
        ...
        session = self.env['eh.reconciliation.session'].open_or_create(self.bank_journal.id)
        line = self.make_statement_line(70.0, partner=partner, payment_ref='Net settlement')
    ...
    def test_inbound_line_rejects_group_that_nets_wrong_side(self):
        ...
        with self.assertRaises(UserError):
            session.apply_match(line.id, (inv_aml + cn_aml).ids, source='bulk')
        ...
        self.assertFalse(line.is_reconciled)
        self.assertEqual(
            self._posted_move_ids_on(self.account_receivable),
            moves_before,
