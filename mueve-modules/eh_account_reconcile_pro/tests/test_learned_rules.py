...
class TestLearnedRules(EhReconcileIntegrationTestCase):
    def _train_match(self, payment_ref, partner):
        ...
        move = self.post_balanced_move(
            [
                {'account': self.account_expense, 'debit': 100.0, 'partner': partner},
                {'account': self.account_cash, 'credit': 100.0},
            ]
        )
        aml = move.line_ids.filtered(lambda l: l.account_id == self.account_expense)
        line = self.make_statement_line(100.0, partner=partner, payment_ref=payment_ref)
    ...
    def test_learns_rule_for_recurring_partner(self):
        ...
        learned = created.filtered(lambda r: r.code == 'learned_acmecorp')
        self.assertTrue(learned)
        self.assertEqual(learned.partner_id, self.partner_a)
        # The learned rule matches a new ACMECORP line (case-insensitive).
        new_line = self.make_statement_line(50.0, payment_ref='ACMECORP NEW BILL')
    ...
    def test_below_support_threshold_creates_nothing(self):
        ...
        self.assertFalse(created.filtered(lambda r: r.code == 'learned_raretoken'))
    ...
    def test_idempotent(self):
        ...
        self.assertFalse(second.filtered(lambda r: r.code == 'learned_acmecorp'))
