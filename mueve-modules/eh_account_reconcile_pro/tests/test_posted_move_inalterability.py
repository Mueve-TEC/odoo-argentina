...
class TestPostedMoveInalterability(EhReconcileIntegrationTestCase):
    ...
    def _snapshot(self, move):
        ...
        return {
            ...
            'line_balances': tuple(sorted(round(l.balance, 2) for l in move.line_ids)),
    ...
    def test_write_off_leaves_posted_move_immutable(self):
        ...
        session.apply_write_off(line.id, self.account_expense.id, label='Bank fee')
        ...
        self.assertNotIn(
            ...
            after['line_accounts'],
            "Write-off account was grafted onto the original posted move; " "the posted move was mutated in place.",
        )
        ...
        self.assertTrue(adjusting, "No adjusting entry carries the write-off reclassification.")
        # The adjusting entry touches both the suspense and write-off
        # accounts and is balanced by construction.
        adj = adjusting[0]
        self.assertIn(self.suspense_account.id, adj.line_ids.mapped('account_id.id'))
        self.assertIn(self.account_expense.id, adj.line_ids.mapped('account_id.id'))
        self.assertAlmostEqual(sum(adj.line_ids.mapped('balance')), 0.0, places=2)
    ...
    def test_reconcile_leaves_posted_move_immutable(self):
        ...
        session = self.env['eh.reconciliation.session'].open_or_create(self.bank_journal.id)
        line = self.make_statement_line(100.0, partner=partner, payment_ref='Invoice payment')
        move = line.move_id
        self.assertEqual(move.state, 'posted')
        before = self._snapshot(move)
        ...
        self.assertTrue(adjusting, "No adjusting entry carries the reconciliation reclassification.")
        adj = adjusting[0]
        self.assertAlmostEqual(sum(adj.line_ids.mapped('balance')), 0.0, places=2)
