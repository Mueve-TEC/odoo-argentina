...
class TestAutoReconcile(EhReconcileIntegrationTestCase):
    ...
    def test_auto_reconcile_high_confidence_match(self):
        ...
        aml = self.make_open_invoice_line(self.partner_a, 500.0, ref='INV500')
        line = self.make_statement_line(500.0, partner=self.partner_a, payment_ref='INV500', journal=self.bank_journal)
    ...
    def test_auto_reconcile_skips_low_confidence(self):
        ...
        line = self.make_statement_line(999.0, partner=self.partner_a, payment_ref='NOMATCH', journal=self.bank_journal)
    ...
    def test_auto_reconcile_no_candidate(self):
        session = self.env['eh.reconciliation.session'].open_or_create(self.bank_journal.id)
        line = self.make_statement_line(123.0, partner=self.partner_b, payment_ref='ORPHAN', journal=self.bank_journal)
