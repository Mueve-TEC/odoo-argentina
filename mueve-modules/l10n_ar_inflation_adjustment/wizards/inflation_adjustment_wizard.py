...
class InflationAdjustmentWizard(models.TransientModel):
    ...
    date_from = fields.Date(string='Fecha Desde', required=True, help='Fecha de inicio del ejercicio fiscal a ajustar.')
    date_to = fields.Date(string='Fecha Hasta', required=True, help='Fecha de cierre del ejercicio fiscal.')
    ...
    def default_get(self, field_list):
        ...
        try:
            ...
            fiscal_dates = company.compute_fiscalyear_dates(today - relativedelta(years=1))
    ...
    def _compute_indices(self):
        ...
        for wizard in self:
            ...
            if wizard.start_index and wizard.end_index:
                wizard.adjustment_factor = ((wizard.end_index / wizard.start_index) - 1) * 100
        ...
        for wizard in self:
            ...
            if wizard.date_from and wizard.date_to:
                if wizard.date_from > wizard.date_to:
                    warnings.append(_('⚠ La fecha de inicio no puede ser posterior a la fecha de fin.'))

                # Verificar que existan todos los índices intermedios
                if wizard.start_index and wizard.end_index:
                    missing = self._check_missing_indices(wizard.date_from, wizard.date_to)
                    if missing:
                        warnings.append(_('⚠ Faltan índices IPC para los siguientes períodos: %s') % ', '.join(missing))
    ...
    def _get_move_line_domain(self):
        ...
        if not non_monetary_tag:
            raise UserError(
                _('No se encontró el tag "No Monetaria". ' 'Verifique que el módulo esté correctamente instalado.')
            )
    ...
    def _get_periods(self):
        ...
        if not self.end_index:
            raise UserError(_('No se encontró el índice IPC para la fecha de cierre (%s).') % self.date_to)
    ...
    def action_confirm(self):
        ...
        if not lines_data:
            raise UserError(_('No hay líneas de ajuste para procesar.'))
