...
class TestMoveLineQueryShape(EhAccountUnitTestCase):
    ...
    def test_multiple_selects_comma_separated(self):
        sql = self._q().select_field('account_id').select_balance_sum().select_count().build()
    ...
    def test_date_range_binds_params(self):
        sql = self._q().select_balance_sum().where_date_range('2026-01-01', '2026-12-31').build()
    ...
    def test_journals_binds_tuple_param(self):
        sql = self._q().select_balance_sum().where_journals([1, 2, 3]).build()
    ...
    def test_account_codes_uses_like_with_percent_suffix(self):
        sql = self._q().select_balance_sum().where_account_codes(['1', '20']).build()
        self.assertIn('JOIN account_account acc', sql.code)
        self.assertIn('acc.code_store', sql.code)
    ...
    def test_order_by_account_field_joins_account(self):
        sql = self._q().select_balance_sum().order_by_account_field('code', 'ASC').build()
        self.assertIn('JOIN account_account acc', sql.code)
        self.assertIn('acc.code_store', sql.code)
    ...
    def test_where_analytic_accounts_uses_jsonb_match(self):
        sql = self._q().select_balance_sum().where_analytic_accounts([3, 7]).build()
    ...
    def test_group_by_field(self):
        sql = self._q().select_field('account_id').select_balance_sum().group_by('account_id').build()
    ...
    def test_order_by_field_with_direction(self):
        sql = self._q().select_balance_sum().order_by('date', direction='DESC').build()
        self.assertIn('ORDER BY', sql.code)
        self.assertIn('aml.date DESC', sql.code)

    def test_limit_and_offset(self):
        sql = self._q().select_balance_sum().limit(50).offset(100).build()
    ...
    def test_offset_zero_omitted(self):
        sql = self._q().select_balance_sum().offset(0).build()
        self.assertNotIn('OFFSET', sql.code)

    def test_limit_none_omitted(self):
        sql = self._q().select_balance_sum().limit(None).build()
    ...
    def test_multi_company_scope(self):
        with self.env.cr.savepoint():
            try:
                ...
                return
        sql = MoveLineQuery(self.env, company_ids=[self.cid, other.id]).select_balance_sum().build()
    ...
    def test_where_raw_appends_fragment(self):
        sql = self._q().select_balance_sum().where_raw(SQL("aml.amount_currency > %s", 0)).build()
    ...
    def test_account_codes_prefix_with_percent_is_treated_as_like_pattern(self):
        ...
        sql = self._q().select_balance_sum().where_account_codes(['1%; DROP TABLE--']).build()
...
class TestMoveLineQueryCurrencyConversion(EhAccountUnitTestCase):
    ...
    * With a multicurrency table attached, the converted sums render the
    ...
    def _multicurrency_table(self, rate_map):
        ...
        ct = CurrencyTable(
            self.env,
            company_ids=list(rate_map.keys()),
    ...
    def test_converted_debit_and_credit_render_rate_multiply(self):
        ...
        sql = q.select_debit_sum_converted().select_credit_sum_converted().build()
    ...
    def test_multicurrency_join_emitted_only_once(self):
        ...
        sql = q.select_balance_sum_converted().select_debit_sum_converted().build()
