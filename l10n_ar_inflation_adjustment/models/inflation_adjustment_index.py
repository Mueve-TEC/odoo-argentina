...
class InflationAdjustmentIndex(models.Model):
    ...
    date = fields.Date(
        string='Fecha',
        required=True,
        help='Primer día del mes al que corresponde el índice IPC.'
    )
    value = fields.Float(
        string='Valor IPC',
        required=True,
    ...
    def _check_first_day_of_month(self):
        ...
        for rec in self:
            ...
            if date_obj.day != 1:
                raise ValidationError(_(
                    'El índice debe corresponder al primer día de cada mes. '
                    'Fecha ingresada: %s'
                ) % rec.date)
    ...
    def _check_positive_value(self):
        ...
        for rec in self:
            if rec.value <= 0:
                raise ValidationError(_(
                    'El valor del índice IPC debe ser un número positivo.'
                ))
